...
--     email that is not in this table. An approval is kept after its account is made.
...
--   * A person can have 5 characters that are not deleted (30 in all, as before).
--
...
-- Supabase Auth runs the hook below as supabase_auth_admin, which reads one column.
...
grant select (email) on table public.approved_emails to supabase_auth_admin;
...
-- Auth calls it for every new account: password sign-up, and a magic link for a new
-- email. '{}' lets the account be made. An error object stops it, and Auth answers the
-- request with that status and message. It runs as supabase_auth_admin, not as definer.
...
as $$
  select case
    when exists (select 1 from public.approved_emails a where a.email = lower(event -> 'user' ->> 'email'))
      then '{}'::jsonb
...
set search_path = public
as $$
  select exists (select 1 from auth.users u where lower(u.email) = p_email);
$$;

-- An approval stays in the table after its account is made. At most 20 can wait
-- for an account at one time. The lock makes two requests at once count one after the other.
create function public.approve_email(p_email text) returns text
language plpgsql
security definer
set search_path = public
as $$
...
begin
  ...
  if exists (select 1 from public.approved_emails a where a.email = v_email) then
    return v_email;
  end if;
  select count(*) into v_waiting
  from public.approved_emails a
  where not public.email_has_account(a.email);
  if v_waiting >= 20 and not public.email_has_account(v_email) then
    raise exception 'there are already 20 approved emails with no account. Revoke one first';
  end if;
  insert into public.approved_emails (email, approved_by) values (v_email, auth.uid());
  return v_email;
end;
$$;

-- Only an approval that no account uses can be revoked.
...
returns table (user_id uuid, email text, display_name text, created_at timestamptz, last_sign_in_at timestamptz, is_admin boolean)
language plpgsql
security definer
stable
set search_path = public
...
-- The approved emails that no account uses yet, oldest first.
create function public.list_pending_approvals()
returns table (email text, approved_at timestamptz, approved_by_name text)
language plpgsql
security definer
stable
set search_path = public
...
begin
  ...
  return query
    select a.email, a.approved_at, p.display_name
    from public.approved_emails a
...
-- ════════════════════════════════════════════════════════════════════
-- Same as 0009 with 5 instead of 10. Bringing a character back counts too.
