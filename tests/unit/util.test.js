...
describe("friendlyError", () => {
  it("maps the database's messages to plain language", () => {
    ...
    expect(friendlyError(new Error("you run this campaign"))).toMatch(/You are the Keeper/);
    ...
    expect(friendlyError(new Error("only the DM of this campaign can create invites"))).toMatch(/Only the Keeper/);
...
describe("safeNextPath", () => {
  it("keeps a path on this site, including its query", () => {
    ...
    expect(safeNextPath("/campaign/x/keeper?tab=1")).toBe("/campaign/x/keeper?tab=1");
  });
